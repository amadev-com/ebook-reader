...
- Stress marks system: `internal/tts/stress.go` — `Stress` store (ai/stress.json global, ai/stress_NNN.json per chapter) maps terms to stressed forms with `+` before the stressed vowel. `Stress.Apply(text)` replaces terms in text. `MergeAll()` merges per-chapter results with conflict detection. Resolved conflicts are marked `Approved` and silently kept on future runs — no re-asking.
